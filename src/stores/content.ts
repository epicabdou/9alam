...
export const useContentStore = defineStore('content', () => {
  ...
  async function generateContent({
    ...
    try {
      ...
      const response = await fetch(
        `${import.meta.env.VITE_SUPABASE_URL}/functions/v1/ai-copywriting`,
        {
          method: 'POST',
          headers: {
    ...
    try {
      ...
      if (contentData.generated_content) {
        ...
        const nextVersionNumber =
          versionsData && versionsData.length > 0 ? versionsData[0].version_number + 1 : 1
  ...
  async function deleteContent(id) {
    ...
    try {
      ...
      const { error: deleteError } = await supabase.from('content_pieces').delete().eq('id', id)
